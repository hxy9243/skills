...
1. **Context Gathering**:
   ...
   - For each provided or discovered update, decide whether it introduces a topic, keyword, connection, contradiction, trend, or key idea worth surfacing. If it does, add or refresh a bullet in `## Emerging Topics & Key Ideas` with a direct wikilink to the relevant note or category. Keep the section to at most 10 bullets, prioritizing explicit invocation updates, then the strongest recent or cross-category ideas.
...
2. **Template Enforcement**:
   ...
   maintain this section manually when the metadata contract is available.)
