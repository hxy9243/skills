...
\\(h\_j\\) is more general than or equal to \\(h\_k\\) (denoted \\(h\_j \\ge\_{g} ph\_k\\)) iff any input instance \\(x\\) that satisfies \\(h\_j\\) also satisfies \\(h\_k\\).
...
\\begin{equation\*} VS\_{H,D} = {h \\in H | h \\text{ is consistent with }D} \\end{equation\*}

- If \\(c \\in H\\), then D can reduce \\(VS\_{H,D}\\) to \\({c}\\).
- If D is insufficient, then \\(VS\_{H,D}\\) represents the *uncertainty* of what the target concept is
- \\(VS\_{H,D}\\) contains all consistent hypotheses, including maximally specific hypotheses

The **general boundary** G of \\(VS\_{H,D}\\) is the set of maximally general members of \\(H\\) consistent with \\(D\\).

The **specific boundary** S of \\(VS\_{H,D}\\) is the set of maximally general members of \\(H\\) consistent with \\(D\\).

\\begin{equation\*} VS\_{H,D} = {h \\in H | \\exists s \\in S \\exists g \\in G g \\ge\_g h \\ge\_g s } \\end{equation\*}
