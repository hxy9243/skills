...
Follow the guidelines: [https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md)
...
std::for_each(myVec.begin(), myVec.end(), [factor](https://braindump.jethro.dev/posts/int& elem){
  elem *= factor;
});
```
...
For `unique_ptr`, there is no reference count within the class. once this unique\_ptr goes out of scope, the object at the pointer is released. For this reason, `unique_ptr` are more lightweight than `shared_ptr`, and cannot be copied. `unique_ptr` can only be moved.
...
[http://stackoverflow.com/questions/1461432/what-is-array-decaying](http://stackoverflow.com/questions/1461432/what-is-array-decaying)
...
Here’s a summary of smart-pointers and their semantics. [value\_ptr](https://github.com/loopperfect/valuable) is not in the stdlib, but is available as a C++ library.

![Figure 1: Tabular summary of smart pointers](https://braindump.jethro.dev/ox-hugo/screenshot_2019-02-24_17-27-51.png)
...
Like shared\_ptr<>, there is no need to call delete or clean the memory in an explicit manner. The unique pointer will take care of it once it goes out of scope. `shared_ptr<>` maintains reference counts where more than one shared\_ptr<> can refer to a single object.
...
In unique\_ptr, std::move is used to transfer the ownership from one unique\_ptr to another, which is otherwise not possible.

For shared\_ptr, std::move prevents the increment and immediate decrement of the reference count, making it an optimization. It’s not strictly necessary.

## not\_null

not\_null is available in GSL, and with compile-time guarantees that a pointer is not null.
