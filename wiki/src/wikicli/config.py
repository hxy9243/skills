...
def load_config(config_path: str | Path | None) -> WikiConfig:
    if config_path is None:
        ...
        return WikiConfig(notebook_root=cwd, generated_root=cwd / "_WIKI", include_roots=(cwd,))
    ...
    generated_root = _resolve_path(raw.get("generated_root", notebook_root / "_WIKI"), base)
    include_roots = tuple(_resolve_path(value, notebook_root) for value in raw.get("include_roots", ["."]))
