...
def run_tournament():
    ...
    opt_run_dir = Path("/home/kevin/Workspace/skills/zettel-eval/output/runs/pairwise_20260329-164451")
    synth_prompt = (opt_run_dir / "best_synthesis_prompt.txt").read_text()
    
    # We didn't optimize the filter prompt in the pairwise run, so we just use the default
    # Or if we want to isolate the synthesis test, we just use the default filter.
    class OptSynth(SynthesizeBrainstorm):
        __doc__ = synth_prompt
        
    optimized.synthesize = dspy.Predict(OptSynth)

    print("Loading 10 random seed notes from the benchmark corpus...", flush=True)
    ...
    for i, ex in enumerate(sample, 1):
        print(f"Match {i}/10: Seed '{ex.source_note_id}'", flush=True)
