...
def parse_packet(raw_packet: str) -> tuple[Packet | None, list[Issue]]:
    try:
        ...
        return None, [Issue("packet_json_invalid", f"packet JSON is invalid: {exc.msg}", line=exc.lineno)]

    if isinstance(payload, list):
        return None, [Issue("packet_not_object", "packet payload must be a single object, not a list")]
    if not isinstance(payload, dict):
        return None, [Issue("packet_not_object", "packet payload must be a JSON object")]
    ...
    search_terms = _string_tuple(payload.get("search_terms", ()), "search_terms", issues)
...
def _required_string(payload: dict[str, Any], key: str, issues: list[Issue]) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        issues.append(Issue("packet_field_invalid", f"packet field must be a non-empty string: {key}"))
        return ""
    return value.strip()
...
def _string_tuple(value: Any, key: str, issues: list[Issue]) -> tuple[str, ...]:
    ...
    if not isinstance(value, (list, tuple)) or not all(isinstance(item, str) for item in value):
        issues.append(Issue("packet_field_invalid", f"packet field must be a list of strings: {key}"))
        return ()
