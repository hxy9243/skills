...
\\begin{equation} \\Delta L(\\boldsymbol{x}\_{k}, t\_{k}) = L(\\boldsymbol{x\_{k}}, t\_{k}) - L(\\boldsymbol{x}\_{k}, t\_{k} - \\Delta t\_{k}) \\end{equation}

reaches a temporal contrast threshold \\(\\pm C\\):

\\begin{equation} \\Delta L(\\boldsymbol{x}\_{k}, t\_{k}) = p\_{k} C \\end{equation}
...
The contrast sensitivity \\(C\\) is determined by pixel bias currents, which set the speed and threshold voltages of the change detector, and are generated by an on-chip digitally programmed bias generator. Typical [[posts-event-based-vision|DVS Cameras]] can set the threshold to 10%-50% illumination change.
...
**Individual Events** \\(e\_{k} = (x\_{k}, t\_{k}, p\_{k})\\) are used by probabilistic filters and [Spiking Neural Networks](https://braindump.jethro.dev/posts/spiking_neurons_lit_review). Filters and SNNs fuse incoming events asynchronously to produce and output. (7, 24, 61, 96, 97)
...
**[Time surface (TS)](https://braindump.jethro.dev/posts/time_surface_ts)** A TS is a 2D map where each pixel stores a single time value. Events are converted into an image whose intensity is a function of the motion history at that location, with brighter values corresponding to more recent motion. TSs highly compress information as they only keep one timestamp per pixel, and their effectiveness degrades on textured scenes, where pixels spike frequently.

**[Voxel Grid](https://braindump.jethro.dev/posts/voxel_grid)** is a space-time (3D) histogram of events, where each voxel represents a particular pixel and time interval. This representation preserves the temporal information of events by avoiding to collapse them on a 2D grid.
...
Deterministic filters such as space-time convolutions and activity filters have been used for noise reduction, feature extraction, image reconstruction, and brightness filtering. Probabilistic filters such as [Kalman Filter](https://braindump.jethro.dev/posts/kalman_filter) and [Particle Filter](https://braindump.jethro.dev/posts/particle_filter) have been used for pose tracking in [SLAM](https://braindump.jethro.dev/posts/slam). Incoming events are compared against additional information to update the filter state.
...
Each event carries little information, and is subject to noise. Hence, it is common to process several events together to yield a sufficient signal-to-noise ratio. [Time Surfaces](https://braindump.jethro.dev/posts/time_surface_ts) are useful for motion analysis and shape recognition, because of their sensitivity to direction of motion and scene edges. Methods using [Voxel Grids](https://braindump.jethro.dev/posts/voxel_grid) involve more memory and computation than lower-dimensional representations.
...
tracking more complex, user-defined shapes can be done using event-by-event adaptations of the [Iterative Closed Point](https://braindump.jethro.dev/posts/iterative_closed_point) algorithm, gradient-descent, mean-shift and [Monte Carlo Methods](https://braindump.jethro.dev/posts/mc_methods), and [Particle Filter](https://braindump.jethro.dev/posts/particle_filter). (118, 119, 176, 177)
