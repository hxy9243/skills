...
\\begin{equation} \\delta\_{i}^{(l)}=\\sigma^{\\prime}\\left(a\_{i}^{(l)}\\right) \\sum\_{k} \\delta\_{k}^{(l+1)} G\_{k i}^{(l)} \\end{equation}
...
\\begin{equation} \\delta\_{i}^{(l)}=\\sigma^{\\prime}\\left(a\_{i}^{(l)}\\right) \\sum\_{k} \\delta\_{k}^{(L)} H\_{k i}^{(l)} \\end{equation}

Random BP applied to [Spiking Neural Networks](https://braindump.jethro.dev/posts/spiking_neurons_lit_review) do not account for the temporal dynamics of neurons and synapses. SuperSpike solves this problem.
