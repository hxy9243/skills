...
- \\(f(n) \\in O(n^c), c > \\log\_ba \\text{ and } \\exists k \\ \\text{ st } af(\\frac{n}{b})\\le kf(n) \\text{ then } T(n) \\in \\Theta(n^{\\log\_ba})\\)
...
![](https://braindump.jethro.dev/ox-hugo/avl_tree.png)
...
- Scan old table \\(O(m\_1)\\), create new table \\(O(m\_2)\\), insert each element \\(O(1)\\), total \\(O(m\_1 + m\_2 + n)\\)
...
- *insert*: \\(T\_1\[h(k)\] = 1\\), \\(T\_2\[h(k)\] = 1\\)
- *search*: if \\(T\_1\[h(k)\]\\) and \\(T\_2\[h(k)\]\\) both 1 return true
...
- Goal: \\(T\_p = (T\_1/p) + T\_\\infty\\), \\(T\_1/p\\) is the parallel part, \\(T\_\\infty\\) is the sequential part

### Matrix Addition

Before: • Work analysis: \\(T\_1(n) = O(n^2)\\) • critical path analysis: \\(T\_\\infty(n) = O(n^2)\\) After:
